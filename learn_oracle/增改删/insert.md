...
```plsql
---要求插入表存在
CREATE TABLE TABLE1 AS SELECT * FROM TABLE2 WHERE 0 = 1;--创建表结构
INSERT INTO TABLE1(COL1,COL2) SELECT COL1,COL2 FROM TABLE2 WHERE CONDITION;--插入部分字段数据
INSERT INTO TABLE1 SELECT * FROM TABLE2 WHERE CONDITION;--插入所有字段
```
